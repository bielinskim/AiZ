...
public class MySort extends SortZ {
    ...
    public void quicksort2(int l, int r, boolean rosnaco) {          // drugi sposob quicksort
        int v = tab[(l + r) / 2];
        int i, j, x;
        i = l;
        j = r;
        do {
            while (this.compare(tab[i], v, rosnaco)) {
                i++;
            }
            while (this.compare(v, tab[j], rosnaco)) {
                j--;
            }
            if (i <= j) {
                x = tab[i];
        ...
        if (j > l) {
            quicksort(l, j, rosnaco);
        }
        if (i < r) {
            quicksort(i, r, rosnaco);
        }
